...
import { SALES_AGENTS, PRODUCT_OFFERS, PRESTATAIRES, MOROCCAN_CITIES } from '../constants';
...
const getCategoryFromOffer = (offre: string) => {
  ...
  if (off.includes('TDLTE')) return 'TDLTE'; 
...
const ADVApp: React.FC<ADVAppProps> = ({ user }) => {
  ...
  const loadData = async () => {
    // Prevent data reload when modal is open to avoid interrupting user work
    if (showModal) {
      return;
    }
    ...
    try {
      ...
      if (data) {
        // CORRECTIF 1: Nettoyer les références au chargement ET normaliser les commerciaux
        const cleanedData = data.map((o: ADVOrder) => {
          // Normalisation du nom commercial pour correspondre à la liste SALES_AGENTS (case-insensitive)
          let normalizedCommercial = (o.commercial || '').trim();
          const canonicalAgent = SALES_AGENTS.find(agent => agent.toLowerCase() === normalizedCommercial.toLowerCase());
          if (canonicalAgent) {
            normalizedCommercial = canonicalAgent;
          }
        ...
        // CORRECTIF 2: Dédupliquer les commandes
        const refMap = new Map<string, ADVOrder>();
        
        const getStatusScore = (o: ADVOrder) => {
            if (o.validation === 'VALIDE') return 100;
            if (o.validation === 'BLOQUÉ') return 50;
            if (o.validation === 'ANNULÉ') return 10;
            if (o.validation === 'EN ATTENTE') return 1;
            return 0;
        };

        for (const order of cleanedData) {
          const key = cleanReference(order.refContrat || '');
          if (!key) continue; // Skip empty refs

          const existing = refMap.get(key);
          if (!existing) {
            refMap.set(key, order);
          } else {
            // CONFLIT DETECTÉ : On garde la version la plus "avancée"
            const scoreExisting = getStatusScore(existing);
            const scoreNew = getStatusScore(order);

            if (scoreNew > scoreExisting) {
                refMap.set(key, order);
            } else if (scoreNew === scoreExisting) {
                const dateExisting = new Date(existing.dateTraitement || 0).getTime();
                const dateNew = new Date(order.dateTraitement || 0).getTime();
        ...
        if (deduplicatedOrders.length < data.length) {
          await saveCloudData('adv_orders', deduplicatedOrders);
        }
      }
    } catch (e) {
  ...
  const runFullSync = async (withConfirm: boolean = true) => {
    // Cette fonction reste strictement inchangée comme demandé
    ...
    try {
      ...
      // 2. Fetch ONLY order IDs/refs from Dolibarr
      ...
      const finalOrders = [...updatedOrders, ...newTransformedOrders];
      
      await saveCloudData('adv_orders', finalOrders);
      setOrders(finalOrders);
      
      setSyncProgress(`✅ +${newTransformedOrders.length} nouvelles commandes ajoutées (total: ${finalOrders.length})`);
      
      setTimeout(() => setSyncProgress(''), 3000);
      return newTransformedOrders.length;
 ...
 const handleSyncDolibarr = async () => {
  ...
  const handleSaveOrder = async () => {
    ...
    const newOrder: ADVOrder = {
      ...
      id: editingOrder ? editingOrder.id : `ADV-${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
    ...
    if (editingOrder) {
      updatedOrders = orders.map(o => o.id === editingOrder.id ? newOrder : o);
    } else {
    ...
    if (formOrder.nSerieVerifie && !editingOrder?.nSerieVerifie) {
      try {
        ...
        if (updatedUnits.some((u: any) => u.serialNumber === formOrder.nSerieVerifie)) {
          await saveCloudData('stock_units', updatedUnits);
        }
      } catch (error) {
    ...
    await saveCloudData('adv_orders', updatedOrders);
  ...
  const handleDeleteOrder = async (id: string) => {
    ...
    if (confirm("Supprimer ce dossier ?")) {
      const updatedOrders = orders.map(o => 
        o.id === id ? { ...o, validation: 'SUPPRIMÉ' as ADVValidationStatus } : o
      );
      setOrders(updatedOrders);
      await saveCloudData('adv_orders', updatedOrders);
    }
  ...
  const handleRestoreOrder = async (id: string) => {
    ...
    if (confirm("Restaurer ce dossier ?")) {
      const updatedOrders = orders.map(o => 
        o.id === id ? { ...o, validation: 'EN ATTENTE' as ADVValidationStatus } : o
      );
      setOrders(updatedOrders);
      await saveCloudData('adv_orders', updatedOrders);
    }
  ...
  const resetFilters = () => {
    ...
    setFilterProduit([]);
    setFilterSousEtatAdv([]);
    setFilterSousEtatSi([]);
    setFilterCategory(null);
  };

  const checkSerialStatus = async (sn: string) => {
    if (!sn) {
        setSerialStatus(null);
        return;
    }
    try {
        const units = await getCloudData('stock_units') as any[] || [];
        const unit = units.find((u: any) => u.serialNumber === sn);
        if (unit) {
            setSerialStatus({ status: unit.status, owner: unit.currentOwner, exists: true });
        } else {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(o => {
      ...
      if (filterCommercial.length > 0) {
        const orderComm = (o.commercial || '').toLowerCase();
        if (!filterCommercial.some(fc => fc.toLowerCase() === orderComm)) return false;
      }
  ...
  const getSerialStatusText = () => {
    ...
    const statusText = statusLabels[serialStatus.status || ''] || serialStatus.status;
  ...
  return (
    <div className="w-full px-6 py-6 h-[calc(100vh-80px)] flex gap-6 items-start">
      ...
      <div className="flex-1 flex flex-col h-full min-0">
        {/* ... (Top Bar / Search / Buttons - remains same) */}
              ...
              <table className="w-full text-left border-collapse">
                 <thead className="bg-white text-[9px] font-black uppercase text-slate-400 sticky top-0 z-10 shadow-sm">
                    <tr>
                       ...
                       <th className="p-4 w-[10%]">Offre / Origine</th>
                    ...
                    {filteredOrders.map(order => (
                       <tr key={order.id} className="hover:bg-slate-50 transition-colors group">
                          <td className="p-4">
                             ...
                             <div className="text-[8px] text-slate-400 font-bold uppercase mt-1">{order.ville || 'Ville inconnue'}</div>
                          </td>
                          <td className="p-4">
                             <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100 truncate max-w-[100px] block uppercase tracking-tight mb-1" title={order.offre}>
                               {order.offre}
                             </span>
                             {order.origine && <span className="text-[7px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{order.origine}</span>}
                          </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
                 ...
                 {/* NOUVEAUX CHAMPS : ORIGINE ET VILLE */}
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Origine</label>
                        <select value={formOrder.origine || 'New'} onChange={e => updateFormOrder({ origine: e.target.value as any })} className="w-full p-4 rounded-xl bg-slate-50 border-none font-black text-sm appearance-none cursor-pointer shadow-inner">
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Ville</label>
                        <select value={formOrder.ville || ''} onChange={e => updateFormOrder({ ville: e.target.value })} className="w-full p-4 rounded-xl bg-slate-50 border-none font-black text-sm appearance-none cursor-pointer shadow-inner">
                 ...
                 <div className="p-6 bg-slate-100/50 rounded-3xl border border-slate-200 grid grid-cols-2 gap-6 relative shadow-sm">
                    ...
                    <div className="space-y-3">
                       ...
                       <select value={formOrder.validation || 'EN ATTENTE'} onChange={e => {
                          ...
                          // AUTOMATISME : Si on valide le dossier ADV, on le pousse automatiquement en 'A traiter' pour l'activation
                          // Sauf si le dossier a déjà un statut technique avancé (pour ne pas écraser un 'Facturé' par exemple)
                          let nextStatutSi = formOrder.statutSi;
                          if (nextValidation === 'VALIDE') {
                              const advancedSiStatuses = ['En GO', 'Installé non Facturé', 'Facturé', 'Bloqué', 'Annulé'];
                              // Si le statut actuel est basique (En Etudes, Non reçu, etc.) ou vide, on force le passage à 'A traiter'
                              if (!nextStatutSi || !advancedSiStatuses.includes(nextStatutSi)) {
                                  nextStatutSi = 'A traiter';
                              }
                          }
                          ...
                          setFormOrder(prev => ({
                            ...
                            statutSi: nextStatutSi, // Application du changement automatique
                    ...
                    <div className="space-y-3">
                       ...
                       <select 
                          value={formOrder.statutSi || 'En Etudes'} 
                          onChange={e => { 
                             const newStatus = e.target.value as ADVSiStatus; 
                             
                             // RÈGLE DE SÉCURITÉ STRICTE : Anti-retour en arrière
                             // Si l'utilisateur tente de remettre "A traiter"
                             if (newStatus === 'A traiter') {
                                 const advancedStatuses = ['En GO', 'Installé non Facturé', 'Facturé', 'Bloqué', 'Annulé'];
                                 // On vérifie le statut REEL en base de données (pas celui du formulaire en cours)
                                 const currentDbStatus = editingOrder?.statutSi;

                                 // Si le dossier est déjà avancé en base, on INTERDIT le retour à "A traiter"
                                 if (currentDbStatus && advancedStatuses.includes(currentDbStatus)) {
                                    alert(`⛔ ACTION INTERDITE\n\nCe dossier est déjà avancé (Statut actuel : "${currentDbStatus}").\n\nIl est strictement impossible de le repasser au statut "A traiter".`);
                                    return; // On annule le changement (le select restera sur l'ancienne valeur visuellement au prochain render)
                                 }
                             }

                             const today = new Date().toISOString().split('T')[0]; 
                             updateFormOrder({ statutSi: newStatus, dateStatutSi: today }); 
                          }} 
                          className="w-full p-3.5 rounded-2xl bg-white border-none font-black text-sm text-slate-700 shadow-inner"
                       >
